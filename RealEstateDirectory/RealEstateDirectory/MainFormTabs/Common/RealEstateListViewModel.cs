...
namespace RealEstateDirectory.MainFormTabs.Common
{
        ...
        where T : RealEstate, new()
    {
                                       ...
                                       IServiceLocator serviceLocator)
        {
            ...
            AvitoOpenFileCommand = new DelegateCommand(OpenPathToAvito);
            ExportToAvitoSelectedCommand = new DelegateCommand(ExportToAvitoSelected);
        }
        ...
        private void LoadFiltersData()
        {
            ...
            RealtorList =
                new ObservableCollection<Realtor>(
                    (new[] {_AllRealtors, _NoneRealtors}).Concat(_RealtorService.GetAll()).ToList());
        ...
        private void UpdateStreet()
        {
            ...
            {
                StreetList =
                    new ObservableCollection<Street>(
                        (new[] {_AllStreets, _NoneStreets}).Concat(district.Streets).ToList());
        ...
        protected void UpdateEntityList()
        {
            ...
            if (dealVariant != null && !Equals(dealVariant, _AllDealVariant))
                entities = Equals(dealVariant, _NoneDealVariant)
                               ...
                               : entities.Where(
                                   room => room.DealVariant != null && Equals(room.DealVariant, dealVariant));
            ...
            entities = needFilter
                           ? entities.Where(
                               room => room.Price.HasValue && room.Price > minPrice && room.Price < maxPrice)
                           : entities;
        ...
        public ICommand ExportToAvitoSelectedCommand { get; private set; }
        public ICommand AvitoOpenFileCommand { get; private set; }
        //Avito
        public int AvitoVariant { get; set; }
        public bool AvitoClear { get; set; }

        protected void ExportToAvitoSelected()
        {
            if (SelectedEntities == null || SelectedEntities.Count == 0)
            {
                _MessageService.ShowMessage("Не выбраны объекты", "Ошибка", image: MessageBoxImage.Error);
            ...
            {
                var variant = 3;
                if (AvitoVariant > 0 && AvitoVariant <= 6)
                {
                    variant = AvitoVariant;
                }

                if (AvitoClear && !_MessageService.ShowConfirm(
                    "Вы выбрали \"Очистить старые данные\": выгруженные и подготовленные ранее объявления будут удалены. Необходимо будет заново начать заполнение объявлений. Продолжить удалени?",
                    "Внимание"))
                {
                    return;
                }

                var avito = new Avito(AvitoClear);
                var data = SelectedEntities.ToList();
                foreach (var estate in data)
                {
                    var ad = GetAd(estate);
                    avito.AddAd(ad, variant);
                }
                avito.Save();

                _MessageService.ShowMessage("Выгрузка выполнена.", "Готово");
            }
        }

        public abstract AdsAD GetAd(RealEstateViewModel<T> item);

        public void SetCommon(AdsAD ad, RealEstateViewModel<T> item)
        {
            ad.Region = "Вологодская область";
            ad.City = "Волгоград";
            ad.AdStatus = "Free";
            ad.Street = item.Address;
            ad.Id = item.Id.ToString();

            ad.OperationType = Avito.Sale;
            ad.Description = item.Description;
            ad.Price = item.Price ?? 0;
            ad.PriceSpecified = true;
        }

        public void OpenPathToAvito()
        {
