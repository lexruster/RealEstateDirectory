...
namespace RealEstateDirectory.Shell
{
    ...
    public class ShellViewModel : NotificationObject
    {
        ...
        public void TestMethod()
        {
            ...
            var str = distrF.Streets.FirstOrDefault();
            if (str != null)
            {
                var name = str.Name;
            }
        ...
        private static void ValidateDictToCount(int count)
        {
            ...
            Debug.Assert(streetService.GetAll().Count() == count);
        ...
        private static void ValidateFlatToCount(int count)
        {
            ...
            Debug.Assert(plotService.GetAll().Count() == count);
        }
        ...
        private static void AddDicitonaryValue(bool addRealEstate = false)
        {
            ...
            var streetEntityNew = new Street("street1", districtEntityNew);
            if (streetService.IsValid(streetEntityNew))
            {
                streetService.Save(streetEntityNew);
            }

            var streetEntityNew1 = new Street("street11", districtEntityNew);
            if (streetService.IsValid(streetEntityNew1))
            {
                streetService.Save(streetEntityNew1);
            }

            var streetEntityNew2 = new Street("street2", districtEntityNew2);
            if (streetService.IsValid(streetEntityNew2))
            {
                streetService.Save(streetEntityNew2);
            }
            var streetEntityNew3 = new Street("street2", districtEntityNew2);
            if (streetService.IsValid(streetEntityNew3))
            {
                streetService.Save(streetEntityNew3);
            }
