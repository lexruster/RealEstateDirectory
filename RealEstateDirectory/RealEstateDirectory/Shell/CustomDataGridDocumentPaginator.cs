...
namespace RealEstateDirectory.Shell
{
	public class CustomDataGridDocumentPaginator : DocumentPaginator
	{
					...
					if (rowIndex > 0)
					{
						...
						if (_documentSource.Columns != null)
						{
							foreach (DataGridColumn column in _documentSource.Columns)
							{
								...
								{
									if (column.Header.ToString() != "Правка")
									{
						...
						if (this.AlternatingRowBorderStyle != null && rowIndex % 2 == 0)
						{
							...
							alernatingRowBorder.BorderThickness = new Thickness(1);
		...
		private DocumentPage ConstructPage(Grid content, int pageNumber)
		{
			...
			return new DocumentPage(pageGrid, PageSize,new Rect(),new Rect(this.PageMargin.Left, this.PageMargin.Top, width, height));
		}
		...
		private void AddTableColumn(Grid grid, double totalColumnWidth, int columnIndex, DataGridColumn column)
		{
				...
				if (column.Header.ToString() == "Комментарий")
				{
					colDefinition.Width = new GridLength(220);
				}
				else if (column.Header.ToString() == "Риэлтор")
				{
					colDefinition.Width = new GridLength(150);
				}
				else
				{
		...
		private void AddTableCell(Grid grid, DataGridColumn column, object item, int columnIndex, int rowIndex)
		{
				...
				if (column is DataGridTemplateColumn)
				{
					...
					contentControl.BorderThickness=new Thickness(0,0,0,1);
					contentControl.BorderBrush = new SolidColorBrush(Color.FromRgb(0,0,0));
				...
				else if (column is DataGridTextColumn)
				{
					...
					text.SetValue(Border.BorderThicknessProperty, new Thickness(0, 0, 0, 1));

					text.SetValue(Border.BorderBrushProperty, new SolidColorBrush(Color.FromRgb(0, 0, 0)));
